  ...
  try {
    ...
    try {
      ...
      if (nextWaiting?.patient?.phone && doc?.hospitalId) {
        const h = await require('../models/Hospital').findById(doc.hospitalId,'name shortName sms');
...
qRouter.post('/reset/:doctorId', protect, authorize('admin', 'staff', 'superadmin'), async (req, res) => {
  try {
    const today = moment().startOf('day').toDate();
    const queue = await Queue.findOneAndUpdate({ doctor: req.params.doctorId, date: today }, { currentNumber: 0, lastAssignedNumber: 0 }, { upsert: true });
    ...
    await logAudit(req, {
      ...
      targetType: 'Queue',
      targetId: queue._id,
      targetName: `Queue for ${doctor?.name || req.params.doctorId}`,
