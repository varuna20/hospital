 ...
 * POST /api/backup/upload            - Upload a backup ZIP and validate it
...
// Multer: store uploaded backup ZIPs in the BACKUP_DIR
const backupStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    // Prefix with "uploaded_" so we can identify it, keep original name
    const safe = file.originalname.replace(/[^a-zA-Z0-9._-]/g, '_');
    cb(null, `uploaded_${Date.now()}_${safe}`);
  }
});
    ...
    const files = fs.readdirSync(BACKUP_DIR)
      ...
      .map(f => {
        ...
        return {
          ...
          isUploaded: f.startsWith('uploaded_')
        };
      })
...
// ── Upload backup file and validate it ─────────────────────────────────────────
router.post('/upload', backupUpload.single('backupFile'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ success: false, message: 'No file uploaded' });

    const fp = req.file.path;

    // Validate: open the ZIP and check for manifest.json with our system signature
    ...
    try {
      ...
      if (!manifestEntry) {
        fs.unlinkSync(fp); // remove invalid file
        return res.status(400).json({ success: false, message: 'Invalid backup: missing manifest.json. This file was not generated by this system.' });
      }
      const buf = await manifestEntry.buffer();
      manifest = JSON.parse(buf.toString('utf-8'));

      // Verify it has our system signature fields
      if (!manifest.version || !manifest.format || !manifest.timestamp || !manifest.collections) {
        fs.unlinkSync(fp);
        return res.status(400).json({ success: false, message: 'Invalid backup: manifest is missing required fields. File may be corrupt or from a different system.' });
      }

      // Check collections list exists
      const colEntries = directory.files.filter(f => f.path.startsWith('collections/') && f.path.endsWith('.ndjson'));
      if (colEntries.length === 0) {
        fs.unlinkSync(fp);
        return res.status(400).json({ success: false, message: 'Invalid backup: no collection data found inside the ZIP.' });
      }
    } catch (zipErr) {
      if (fs.existsSync(fp)) fs.unlinkSync(fp);
      return res.status(400).json({ success: false, message: 'Invalid backup file: could not open as ZIP. ' + zipErr.message });
    }

    const stat = fs.statSync(fp);
    res.json({
      success: true,
      message: 'Backup file validated successfully. Ready to restore.',
      backup: {
...
// ── Helper: run the actual restore logic ──────────────────────────────────────
async function performRestore(fp) {
  const safe = path.basename(fp);
